...
def main():
    ...
    # load data
    train_loader, vocab_words, vocab_answers, index_unk_answer = loaders_factory.get_vqa_loader('train', config, rels=reli)
    
    val_loader = loaders_factory.get_vqa_loader('val', config, rels=reli)

    test_loader = loaders_factory.get_vqa_loader('test', config, rels=reli)
    ...
    # infer test set
    metrics, results = test(test_loader, model, criterion, device, 0, config, None, consistency_term=consisterm, rels=reli) 
    ...
    metrics_val, results_val = validate(val_loader, model, criterion, device, 2000, config, None, consistency_term=consisterm, rels=reli)
    ...
    metrics_train, results_train = validate(train_loader, model, criterion, device, 1000, config, None, consistency_term=consisterm, rels=reli)
    ...
    metrics_val, results_val = validate(val_loader, model, criterion, device, 2001, config, None, consistency_term=consisterm, rels=reli)
    ...
    metrics_train, results_train = validate(train_loader, model, criterion, device, 1001, config, None, consistency_term=consisterm, rels=reli)
