...
def get_vqa_loader(subset, config, shuffle=False, draw_borders=False, rels=False):
    ...
    if config['dataset']=='vqa2':
        ...
        dataset_vqa = vqa.VQA2MainSub(subset, config, dataset_visual, rels=rels)
    elif config['dataset']=='gqa':
        dataset_vqa = vqa.GQAMainSub(subset, config, dataset_visual)
    elif 'idrid_regions_single' in config['dataset'] or 'vqa2_regions_single' in config['dataset'] or 'localized' in config['dataset']:
        dataset_mask = visual.get_mask_dataset(subset, config, 'maskA')
        if config['mainsub'] and subset != 'test': # if main and sub-questions, use corresponding dataset class
            dataset_vqa = vqa.VQARegionsSingleMainSub(subset, config, dataset_visual, dataset_mask, draw_borders=draw_borders, rels=rels)
    ...
    if config['mainsub'] and config['dataset'] == 'gqa':
        collater_fn = collater_mainsub
