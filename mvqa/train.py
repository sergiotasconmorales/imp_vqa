...
def main():
    ...
    if 'rels' in config:
        ...
        reli = False
    train_loader, vocab_words, vocab_answers, index_unk_answer = loaders_factory.get_vqa_loader('train', config, shuffle=True, rels = reli) 
    ...
    for epoch in range(start_epoch, config['epochs']+1):
        ...
        # train for one epoch
        train_epoch_metrics = train(train_loader, model, criterion, optimizer, device, epoch, config, logbook, comet_exp=comet_experiment, consistency_term=consisterm, rels=reli)
        ...
        val_epoch_metrics, val_results = validate(val_loader, model, criterion, device, epoch, config, logbook, comet_exp=comet_experiment, consistency_term=consisterm, rels=reli)
